...
function handleEditTodoSubmit(todoId) {
    ...
    todoList[findIndex].content = document.querySelector(".todo-input").value; //수정된 값을 담수 실행문
    todoList[findIndex].date = convertDateKor(new Date()); // 수정된 날짜를 담는 실행문

    localStorage.setItem("todoList", JSON.stringify(todoList)); // 수정된 문자열을 set하는 실행문
    getTodoList();
